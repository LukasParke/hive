<script lang="ts">
	...
	import type { SwarmNode, NodeAllDisks, BlockDevice } from '$lib/types';
	...
	let nodes = $state<SwarmNode[]>(pageData.nodes ?? []);
	...
	function getNodeHostname(nodeId: string): string {
		...
		return node?.Description.Hostname || nodeId;
	}

	function getNodeIP(nodeId: string): string {
		const node = nodes.find(n => n.ID === nodeId);
		return node?.Status.Addr?.replace(':2377', '') || '';
	}
...
<svelte:head><title>Deploy Ceph | Hive</title></svelte:head>

<div>
	<div class="flex items-center gap-4 mb-6">
		<a href="/ceph" style="color: var(--color-muted); text-decoration: none;">← Back</a>
					...
					{#each nodes as node}
						...
						>
							<div class="flex items-center justify-between">
								<div>
<span class="font-medium">{node.Description?.Hostname}</span>
							<span class="text-sm ml-2" style="color: var(--color-muted);">{node.Status?.Addr}</span>
								</div>
								<div class="flex items-center gap-3">
