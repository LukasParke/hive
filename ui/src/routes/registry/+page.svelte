...
<svelte:head><title>Container Registry | Hive</title></svelte:head>
	...
	{#if data.status}
		<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
			<div class="rounded-lg p-4" style="background-color: var(--color-surface); border: 1px solid var(--color-border);">
				...
				<p class="text-lg font-semibold" style="color: {data.status.running ? 'var(--color-success)' : 'var(--color-danger)'};">
			...
			<div class="rounded-lg p-4" style="background-color: var(--color-surface); border: 1px solid var(--color-border);">
				...
				<p class="text-lg font-semibold">{data.status.image_count ?? 0}</p>
			</div>
			<div class="rounded-lg p-4" style="background-color: var(--color-surface); border: 1px solid var(--color-border);">
				...
				<div class="flex flex-wrap gap-2">
					{#each image.tags ?? [] as tag}
						<div class="flex items-center gap-1 px-2 py-1 rounded text-xs" style="background-color: var(--color-bg); border: 1px solid var(--color-border);">
							...
							<button onclick={() => deleteImage(image.name, tag)} class="ml-1" style="color: var(--color-danger);">x</button>
						</div>
					{/each}
				</div>
			</div>
