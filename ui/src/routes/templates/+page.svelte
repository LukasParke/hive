<script lang="ts">
	...
	import type { TemplateSource, CustomTemplate } from '$lib/types';
...
<svelte:head><title>Templates | Hive</title></svelte:head>
...
<div class="max-w-4xl mx-auto">
	...
	<div class="rounded-lg p-4" style="background-color: var(--color-surface); border: 1px solid var(--color-border);">
		...
		<div class="space-y-3">
			{#each data.customTemplates ?? [] as ct}
				<div
					class="flex items-center justify-between p-3 rounded-lg"
					style="background-color: var(--color-bg); border: 1px solid var(--color-border);"
				>
			...
			{/each}
			{#if (data.customTemplates ?? []).length === 0}
